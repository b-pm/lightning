defmodule Lightning.Workflows.Stats do
  ...
  # The trade is that a busy workflow recomputes on every poll, measured
  # harmless (`.context/frank/notes/workflow-health-explain.sql` §10).
  #
  ...
  # `last_activity` is stamped on every state change and autogenerated on
  # insert (`Lightning.WorkOrder`), so dropping that autogenerate would
  # silently freeze this page's numbers. Indexed as
  # `(workflow_id, last_activity)`, so this is a lookup, not a scan.
